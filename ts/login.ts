...
function obtenerElementoFormulario<T extends HTMLElement>(
  id: string,
  tipo: new () => T,
): T {
...
function inicializarAnimacionPanda(): void {
  let formElements: LoginFormElements;

  try {
    formElements = obtenerInputsFormulario();
  } catch {
    return;
  }

  const { login, password } = formElements;
...
formulario.addEventListener("submit", (event: SubmitEvent) => {
  ...
  if (/^[\d._-]/.test(loginValue)) {
    event.preventDefault();
    alert(
      "El nombre de usuario no puede comenzar por un número, guion, guion bajo o punto.",
    );
    return;
  }
  ...
  if (
    ...
    event.preventDefault();
    alert(
      "La contraseña debe tener al menos una letra mayúscula, una letra minúscula, un número y tres caracteres especiales distintos.",
    );
    return;
  }
  ...
  if (
    ...
    event.preventDefault();
    alert(
      'La contraseña no puede contener secuencias alfabéticas, de caracteres especiales o numéricas inseguras como "abc", "qwerty", "qaz", "123", "147", "159"',
    );
  }
});
