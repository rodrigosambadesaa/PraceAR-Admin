...
const tecladoEspanolFilas = ["qwertyuiop", "asdfghjklñ", "zxcvbnm"];
...
export async function haSidoFiltradaEnBrechas(
  password: string,
): Promise<boolean> {
...
export function contrasenhaSimilarAUsuario(
  contrasenha: string,
...
export function tieneSecuenciasNumericasInseguras(
  contrasenha: string,
): boolean {
  ...
  for (let longitud = 2; longitud <= 5; longitud += 1) {
    for (let i = 0; i <= numeros.length - longitud; i += 1) {
      ...
      secuenciasNumericasInseguras.push(
        numerosReverso.substring(i, i + longitud),
      );
    }
  }
  ...
  return secuenciasNumericasInseguras.some((secuencia) =>
    contrasenha.includes(secuencia),
  );
}

export function tieneSecuenciasAlfabeticasInseguras(
  contrasenha: string,
): boolean {
  ...
  for (let longitud = 2; longitud <= 5; longitud += 1) {
    for (let i = 0; i <= alfabeto.length - longitud; i += 1) {
      ...
      secuenciasAlfabeticasInseguras.push(
        alfabetoReverso.substring(i, i + longitud),
      );
    }
  }
  ...
  for (const fila of tecladoEspanolFilas) {
    for (let longitud = 2; longitud <= fila.length; longitud += 1) {
      ...
      for (let i = 0; i <= fila.length - longitud; i += 1) {
        ...
        secuenciasAlfabeticasInseguras.push(
          filaReverso.substring(i, i + longitud),
        );
      }
    }
  }

  const secuenciasDiagonalesTecladoEspanolReverso =
    secuenciasDiagonalesTecladoEspanol.map((secuencia) =>
      secuencia.split("").reverse().join(""),
    );
  secuenciasAlfabeticasInseguras.push(
  ...
  return secuenciasAlfabeticasInseguras.some((secuencia) =>
    contrasenha.includes(secuencia),
  );
}

export function tieneSecuenciasDeCaracteresEspecialesInseguras(
  contrasenha: string,
): boolean {
