...
export class SecurePasswordGenerator {
  ...
  private static ALPHA_SEQS: string[] =
    SecurePasswordGenerator.computeAlphaSeqs();
  ...
  static verifyStrongPassword(password: string): boolean {
    ...
    for (const c of password)
      if ("!@#$%^&*()-_=+[]{}|;:,.<>?/".includes(c)) especiales.add(c);
  ...
  static generarPassword(length: number): string {
    if (length > 256) {
      ...
      const longitudParte = Math.floor(length / partes),
        resto = length % partes;
      while (true) {
        let pwd = "";
  ...
  static getStats(password: string): PasswordStats {
    ...
    const entropy =
      lowercase * 5 + uppercase * 6 + digits * 7 + special * 8 + " bits";

    // Usar BigInt y exponentiación para evitar overflow de Number/Infinity
    const totalCombinations =
      26n ** BigInt(lowercase + uppercase) *
      10n ** BigInt(digits) *
      32n ** BigInt(special);
    ...
    const yearsToCrack =
      totalCombinations / (attemptsPerSecond * secondsInYear);
    ...
    if (yearsToCrack < 1_000n) {
      ...
      resistanceTimeToBruteForceAttack =
        (yearsToCrack / 1_000n).toString() + " mil años";
    } else {
      const millones = yearsToCrack / 1_000_000n;
      resistanceTimeToBruteForceAttack =
        millones.toString() + " millones de años";
    }
