...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($content_length !== null && $content_length > $max_request_body_bytes) {
        ...
        $err =
            '<span style="color: red; text-align: center;">La solicitud supera el tamaño máximo permitido.</span>';
    ...
    if ($err === "") {
        try {
            ...
            $err =
                '<span style="color: red; text-align: center;">Se superó el límite de intentos. Espere ' .
                htmlspecialchars(
                    (string) $exception->getRetryAfter(),
                    ENT_QUOTES,
                    "UTF-8",
                ) .
                " segundos antes de volver a intentarlo.</span>";
            error_log(
                sprintf(
    ...
    if ($err === "") {
        try {
            ...
            if (tiene_espacios_al_principio_o_al_final($_POST["password"])) {
                throw new Exception(
                    "La contraseña no puede tener espacios al principio o al final.",
                );
            }

            if (strlen($password) < 16 || strlen($password) > 1024) {
                throw new Exception(
                    "La contraseña debe tener entre 16 y 1024 caracteres.",
                );
            }
            ...
            if ($result->num_rows === 1) {
                ...
                for ($i = 0; $i < count($pepper_config); $i++) {
                    if (
                        ...
                        $insert_sql =
                            "INSERT INTO accesos (id, id_usuario, ip, user_agent, fecha, tipo) VALUES (NULL, ?, ?, ?, ?, ?)";
