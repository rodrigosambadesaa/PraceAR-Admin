...
function withUpdatedQueryParam(urlValue, key, value) {
...
function synchronizeLanguageContext(languageCode) {
    if (inputLang) {
    ...
    if (inputDeshacer instanceof HTMLInputElement) {
        ...
        if (currentRedirect) {
            inputDeshacer.dataset.redirectUrl = withUpdatedQueryParam(currentRedirect, "lang", languageCode);
        }
    }
    document
        .querySelectorAll("[data-codigo_idioma]")
        .forEach((element) => {
        element.dataset.codigo_idioma = languageCode;
    });
    document
        .querySelectorAll("a[data-translation-link='true']")
        .forEach((link) => {
        link.href = withUpdatedQueryParam(link.href, "codigo_idioma", languageCode);
        const flagImage = link.querySelector("img");
        if (flagImage) {
            // @ts-ignore
...
function initializeTableLanguageSelector() {
...
function validateQuickEditForm(form, field) {
    ...
    switch (field) {
        ...
        case "imagen": {
            ...
            const deleteChecked = form.querySelector("input[name='eliminar_imagen']")
                ?.checked ?? false;
...
function overrideQuickEditSubmit(form, context) {
    form.onsubmit = (event) => {
        event.preventDefault();
        const fieldForValidation = form.querySelector("[name='field']")?.value ??
            context.field;
