...
formulario.addEventListener("submit", (event: SubmitEvent) => {
  ...
  if (busqueda && /[<>"'%]/.test(busqueda)) {
    ...
    alert(
      "Por favor, elimine los caracteres no permitidos, que son: <, >, \", ', %",
    );
  }
});
...
const zoomElements = {
  ...
  closeButton: document.querySelector<HTMLButtonElement>(
...
function initializeZoomableImages(): void {
  const zoomableImages =
    document.querySelectorAll<HTMLImageElement>(".zoomable");
  zoomableImages.forEach(attachZoomableImage);
}

function setQuickEditMessage(
  message: string,
  type: QuickEditMessageType = "info",
): void {
  const messageElement =
    modalElements.body?.querySelector<HTMLElement>("#quick-edit-msg");
  ...
  switch (field) {
    ...
    case "telefono": {
      ...
      if (!value) {
        ...
        message =
          "Introduce un teléfono válido (solo números, espacios y los símbolos +, -, ( )).";
      }
      break;
    }
    case "caseta_padre": {
      ...
      if (value && !/^[A-Za-z0-9_-]{2,15}$/.test(value)) {
        message =
          "La caseta padre debe tener entre 2 y 15 caracteres alfanuméricos, guiones o guiones bajos.";
      }
      break;
    }
    case "traduccion": {
    ...
    case "imagen": {
      ...
      const deleteChecked =
        form.querySelector<HTMLInputElement>("input[name='eliminar_imagen']")
          ?.checked ?? false;
...
function updateCellAfterSuccess(
  context: QuickEditTargetInfo,
  form: HTMLFormElement,
): void {
  ...
  switch (field) {
    ...
    case "descripcion": {
      const descripcionInput = form.querySelector<HTMLTextAreaElement>(
        "[name='descripcion']",
      );
      if (descripcionInput) {
    ...
    case "traduccion": {
      ...
      const descripcionCell = row?.querySelector<HTMLElement>(
      ...
      if (descripcionCell && descripcionInput) {
        descripcionCell.textContent = truncateText(
    ...
    case "imagen": {
      ...
      const existingImage =
        cell.querySelector<HTMLImageElement>("img.zoomable");
      ...
      if (fileInput && fileInput.files && fileInput.files.length > 0) {
        const row = cell.closest("tr");
        const casetaCell = row?.querySelector<HTMLElement>(
          "[data-field='caseta']",
        );
...
function overrideQuickEditSubmit(
  form: HTMLFormElement,
  context: QuickEditTargetInfo,
): void {
  form.onsubmit = (event) => {
...
function handleEditableClick(element: HTMLElement): void {
  ...
  fetch("admin/ajax_quick_edit.php", {
    ...
    .then((html) => {
      ...
      const possibleForm =
        modalElements.body.querySelector<HTMLFormElement>("form");
      if (possibleForm) {
