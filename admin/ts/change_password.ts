...
const csrfInput = formulario.querySelector('input[name="csrf"]') as HTMLInputElement | null;

const generatorToggleButton = document.getElementById("toggle-password-generator") as HTMLButtonElement | null;
const generatorPanel = document.getElementById("password-generator-panel") as HTMLElement | null;
const lengthNumberInput = document.getElementById("password-length-number") as HTMLInputElement | null;
const lengthRangeInput = document.getElementById("password-length-range") as HTMLInputElement | null;
const lengthOutput = document.getElementById("password-length-output") as HTMLElement | null;
const generateButton = document.getElementById("generate-password-button") as HTMLButtonElement | null;
const generatorFeedback = document.getElementById("password-generator-feedback") as HTMLElement | null;
const generatedPasswordContainer = document.getElementById("generated-password-container") as HTMLElement | null;
const generatedPasswordValue = document.getElementById("generated-password-value") as HTMLElement | null;
const copyButton = document.getElementById("copy-generated-password") as HTMLButtonElement | null;
const copyFeedback = document.getElementById("password-copy-feedback") as HTMLElement | null;
const resistanceElement = document.getElementById("password-length-resistance") as HTMLElement | null;
const statsElements = {
    uppercase: document.getElementById("password-stat-uppercase") as HTMLElement | null,
    lowercase: document.getElementById("password-stat-lowercase") as HTMLElement | null,
    digits: document.getElementById("password-stat-digits") as HTMLElement | null,
    special: document.getElementById("password-stat-special") as HTMLElement | null,
    hashResistance: document.getElementById("password-stat-hash") as HTMLElement | null,
    entropy: document.getElementById("password-stat-entropy") as HTMLElement | null,
};

function setGeneratorFeedback(message: string, type: "info" | "error" | "success" = "info") {
...
function calculateLengthResistance(length: number): string {
    if (length >= 16 && length <= 20) return "Resistente a atacantes individuales.";
    if (length > 20 && length <= 30) return "Resistente a grupos pequeños de atacantes.";
    if (length > 30 && length <= 50) return "Resistente a empresas con recursos moderados.";
    if (length > 50 && length <= 70) return "Resistente a grandes empresas.";
    if (length > 70) return "Resistente a gobiernos y organizaciones con recursos avanzados.";
    return "Longitud insuficiente para garantizar resistencia.";
}

function updateResistance() {
    if (!resistanceElement || !(lengthNumberInput instanceof HTMLInputElement)) return;
...
function updateLengthOutput(value: string) {
    if (lengthOutput) lengthOutput.textContent = value;
    updateResistance();
}

function syncLengthFromNumber() {
    if (!(lengthNumberInput instanceof HTMLInputElement) || !(lengthRangeInput instanceof HTMLInputElement)) return;
...
function toggleGeneratorPanel() {
    if (!(generatorPanel instanceof HTMLElement) || !(generatorToggleButton instanceof HTMLButtonElement)) return;
    ...
    if (!generatorPanel.hasAttribute("hidden")) updateResistance();
}
...
async function handleGenerateClick() {
    if (!(lengthNumberInput instanceof HTMLInputElement) || !(generateButton instanceof HTMLButtonElement)) return;
    const lengthValue = Number.parseInt(lengthNumberInput.value, 10);
    if (Number.isNaN(lengthValue)) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Respuesta inesperada (${response.status})`);
...
function handleCopyPassword() {
    if (!(generatedPasswordValue instanceof HTMLElement) || !generatedPasswordValue.textContent) return;
