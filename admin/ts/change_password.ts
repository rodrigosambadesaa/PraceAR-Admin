...
const generatorToggleButton = document.getElementById(
  "toggle-password-generator",
) as HTMLButtonElement | null;
const generatorPanel = document.getElementById(
  "password-generator-panel",
) as HTMLElement | null;
const lengthNumberInput = document.getElementById(
  "password-length-number",
) as HTMLInputElement | null;
const lengthRangeInput = document.getElementById(
  "password-length-range",
) as HTMLInputElement | null;
const lengthOutput = document.getElementById(
...
const generatedPasswordValue = document.getElementById(
...
const resistanceElement = document.getElementById(
  "password-length-resistance",
) as HTMLElement | null;
const statsElements = {
  uppercase: document.getElementById(
    "password-stat-uppercase",
  ) as HTMLElement | null,
  lowercase: document.getElementById(
    "password-stat-lowercase",
  ) as HTMLElement | null,
  digits: document.getElementById("password-stat-digits") as HTMLElement | null,
  special: document.getElementById(
    "password-stat-special",
  ) as HTMLElement | null,
  hashResistance: document.getElementById(
    "password-stat-hash",
  ) as HTMLElement | null,
  entropy: document.getElementById(
    "password-stat-entropy",
  ) as HTMLElement | null,
};

function setGeneratorFeedback(
  message: string,
  type: "info" | "error" | "success" = "info",
) {
  if (!generatorFeedback) return;
  generatorFeedback.textContent = message;
  generatorFeedback.classList.remove(
    "admin-error-text",
    "admin-success-text",
    "admin-info-text",
  );
...
function syncLengthFromNumber() {
  if (
    !(lengthNumberInput instanceof HTMLInputElement) ||
    !(lengthRangeInput instanceof HTMLInputElement)
  )
    return;
...
function updateGeneratorToggleState(isExpanded: boolean) {
  if (!(generatorToggleButton instanceof HTMLButtonElement)) {
    return;
  }

  generatorToggleButton.setAttribute(
    "aria-expanded",
...
async function handleGenerateClick() {
  ...
  if (Number.isNaN(lengthValue)) {
    setGeneratorFeedback(
      "La longitud debe ser un número natural válido.",
      "error",
    );
  ...
  if (lengthValue < 16 || lengthValue > 1024) {
    setGeneratorFeedback(
      "La longitud debe estar entre 16 y 1024 caracteres.",
      "error",
    );
    resetGeneratedPassword();
    return;
  ...
  if (!csrfToken) {
    setGeneratorFeedback(
      "No se pudo validar la petición (token CSRF no disponible).",
      "error",
    );
    resetGeneratedPassword();
    return;
  ...
  try {
    ...
    if (
      !(generatedPasswordContainer instanceof HTMLElement) ||
    ...
    if (!password) {
      setGeneratorFeedback(
        "No se recibió ninguna contraseña generada.",
        "error",
      );
      resetGeneratedPassword();
      return;
...
function handleCopyPassword() {
  if (
    ...
    return;
  navigator.clipboard
    .writeText(generatedPasswordValue.textContent)
    .then(() => {
      if (copyFeedback instanceof HTMLElement) {
        resetCopyFeedback();
...
formulario.addEventListener("submit", (event: SubmitEvent) => {
  ...
  const longitudInvalida = (valor: string) =>
    valor.length < 16 || valor.length > 1024;
  if (
  ...
  if (
    ...
    event.preventDefault();
    alert(
      "La nueva contraseña debe tener al menos una letra mayúscula, una letra minúscula, un número y tres caracteres especiales distintos.",
    );
    return;
  }
  if (
    ...
    event.preventDefault();
    alert(
      'La nueva contraseña no puede contener secuencias alfabéticas, de caracteres especiales o numéricas inseguras como "abc", "qwerty", "qaz", "123", "147", "159"',
    );
  }
});
...
updateGeneratorToggleState(
  generatorPanel instanceof HTMLElement && !generatorPanel.hasAttribute("hidden"),
);
