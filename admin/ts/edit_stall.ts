...
function getOptionalInput(id: string): HTMLInputElement | null {
  ...
  if (!(element instanceof HTMLInputElement)) {
    throw new Error(
      `El elemento con id "${id}" no es un campo de texto válido.`,
    );
  }
...
function validateCasetaPadre(casetaPadre: string): string | null {
  ...
  if (
    !letrasValidas.has(letrasPadre) ||
    numeroPadreValor < 1 ||
    numeroPadreValor > 370
  ) {
...
if (
  eliminarImagenLink instanceof HTMLAnchorElement &&
  eliminarImagenInput instanceof HTMLInputElement
) {
...
function openZoom(image: HTMLImageElement): void {
  if (
    !(zoomedContainer instanceof HTMLElement) ||
    !(zoomedImage instanceof HTMLImageElement)
  ) {
    return;
  }
...
function limpiarMensajesPrevios(): void {
  ...
  if (
    siguienteElemento instanceof HTMLDivElement &&
    siguienteElemento.classList.contains("form-errors")
  ) {
...
formulario.addEventListener("submit", async (event: SubmitEvent) => {
  ...
  const eliminarImagen = eliminarImagenInput
    ? limpiarInput(eliminarImagenInput.value)
    : "";
  ...
  const casetaPadre = casetaPadreInput
    ? limpiarInput(casetaPadreInput.value)
    : "";
    ...
    if (!allowedTypes.has(archivo.type)) {
      ...
      try {
        ...
        if (!verification.success) {
          errores.push(
            verification.message || "No se pudo verificar la imagen.",
          );
        ...
        errores.push(
          "Error al verificar la imagen. Por favor, inténtelo de nuevo.",
        );
      }
    }
  }
