...
$codigo_idioma = isset($input["codigo_idioma"])
    ? limpiar_input($input["codigo_idioma"])
    : "";
...
if (in_array($field, ["nombre", "contacto", "telefono", "caseta_padre"])) {
    ...
    echo "<form id='quick-edit-form' enctype='multipart/form-data'>
        ...
        <input type='text' id='quick-edit-input' name='value' value='" .
...
if ($field === "imagen") {
    ...
    $imgHtml = file_exists(dirname(__DIR__) . "/$ruta")
        ? "<img src='/$ruta' style='max-width:200px;max-height:200px;'>"
...
if (in_array($field, ["tipo", "descripcion", "traduccion"])) {
    ...
    echo "<form id='quick-edit-trad-form'>
        ...
        <input type='hidden' name='codigo_idioma' value='" .
        htmlspecialchars($codigo_idioma) .
        "'>
        <label for='quick-edit-tipo'>Tipo</label>
        <input type='text' id='quick-edit-tipo' name='tipo' value='" .
        htmlspecialchars($row["tipo"]) .
        "' required><br>
        <label for='quick-edit-descripcion'>Descripción</label>
        <textarea id='quick-edit-descripcion' name='descripcion' maxlength='450'>" .
