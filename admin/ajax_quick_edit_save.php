...
if ($field === "imagen") {
    ...
    if (
        ...
        if (!$check["success"]) {
            response(
                false,
                "Error al analizar la imagen: " . $check["message"],
            );
        }
        if ($check["is_malicious"]) {
            response(
                false,
                "La imagen es potencialmente maliciosa: " . $check["message"],
            );
        }
...
if ($field === "traduccion" || $field === "tipo" || $field === "descripcion") {
    $codigo_idioma = isset($_POST["codigo_idioma"])
        ? limpiar_input($_POST["codigo_idioma"])
        : "";
    $tipo = isset($_POST["tipo"]) ? limpiar_input($_POST["tipo"]) : "";
    $descripcion = isset($_POST["descripcion"])
        ? limpiar_input($_POST["descripcion"])
        : "";
    $sql =
        "UPDATE puestos_traducciones SET tipo = ?, descripcion = ? WHERE codigo_idioma = ? AND puesto_id = ?";
    $stmt = $conexion->prepare($sql);
