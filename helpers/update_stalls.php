...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (
        ...
        if (
            ...
            throw new Exception(
                "El archivo no es una imagen válida o es demasiado grande. Por favor, suba un archivo con extensión .jpg o .jpeg y un tamaño máximo de 2MB.",
            );
        }
        ...
        throw new Exception(
            "Error al subir la imagen. Código de error: " .
                $_FILES["imagen"]["error"],
        );
    }
    ...
    if (isset($eliminar_imagen) && is_string($eliminar_imagen)) {
        try {
            ...
            throw new Exception(
                "Error al convertir eliminar_imagen a entero: " .
                    $e->getMessage(),
            );
        }
    }
    ...
    if (
        !empty($caseta_padre) &&
        (!is_string($caseta_padre) || strlen($caseta_padre) !== 5)
    ) {
        throw new Exception(
            "La caseta padre debe ser un texto de exactamente cinco caracteres",
        );
    }
    ...
    if (!empty($caseta_padre)) {
        if (!preg_match('/^(CE|CO|MC|NA|NC)([0-9]{3})$/', $caseta_padre)) {
            throw new Exception(
                "La caseta padre debe tener el formato correcto. El formato correcto es dos letras seguidas de tres números. Las dos letras deben ser 'CE', 'CO', 'MC', 'NA', o 'NC', y los tres números deben estar entre 001 y 370.",
            );
        }
    ...
    if ($stmt->execute()) {
        $mensaje =
            "<span id='mensaje_correcto'>Puesto actualizado correctamente</span>";
        $stmt->close();
        $conexion->close();
