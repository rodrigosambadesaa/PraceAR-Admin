 ...
 */
function es_contrasenha_antigua(
    string $contrasenha_antigua_a_verificar,
    ...
    if (!empty($missingConfiguration)) {
        error_log(
            "Faltan variables de configuración para verificar contraseñas antiguas: " .
                implode(", ", $missingConfiguration),
        );
        throw new RuntimeException(
            "No se pudo verificar el historial de contraseñas. Contacte con la persona administradora.",
        );
    }
    ...
    try {
        ...
        error_log(
            "Error al consultar contraseñas antiguas: " .
...
function ha_sido_filtrada_en_brechas_de_seguridad(string $contrasenha): bool
{
    ...
    foreach ($hashes as $hash) {
        ...
        if (
            isset($hash_parts[0]) &&
            strtoupper($hash_parts[0]) === strtoupper($hash_suffix)
        ) {
            return true;
        }
    }
 ...
 */
function contrasenha_similar_a_usuario(
    string $contrasenha,
    array|string $usuario,
): bool {
 ...
 */
function tiempo_estimado_resistencia_ataque_fuerza_bruta(
    string $contrasenha,
): string {
    /*
    ...
    if (!extension_loaded("bcmath")) {
        throw new Exception(
            "La extensión BCMath no está habilitada en este servidor.",
        );
    }
...
function tiene_secuencias_numericas_inseguras(string $contrasenha): bool
{
    ...
    for ($longitud = 2; $longitud <= 5; $longitud++) {
        for ($i = 0; $i <= min(strlen($numeros), 20) - $longitud; $i++) {
            ...
            $secuencias_numericas_inseguras[] = substr(
                $numeros_reverso,
                $i,
                $longitud,
            );
...
function tiene_secuencias_alfabeticas_inseguras(string $contrasenha): bool
{
    ...
    $filas_teclado_espanol = [
        $fila_superior_teclado_espanol,
        $fila_media_teclado_espanol,
        $fila_inferior_teclado_espanol,
    ];
    ...
    for ($longitud = 2; $longitud <= 5; $longitud++) {
        for ($i = 0; $i <= strlen($alfabeto) - $longitud; $i++) {
            $secuencias_alfabeticas_inseguras[] = substr(
                $alfabeto,
                $i,
                $longitud,
            );
    ...
    $secuencias_diagonales_teclado_espanol_reverso = array_map(
        "strrev",
        $secuencias_diagonales_teclado_espanol,
    );
    $secuencias_alfabeticas_inseguras = array_merge(
        $secuencias_alfabeticas_inseguras,
        $secuencias_diagonales_teclado_espanol,
        $secuencias_diagonales_teclado_espanol_reverso,
    );
 ...
 */
function tiene_secuencias_caracteres_especiales_inseguras(
    string $contrasenha,
): bool {
 ...
 */
function contrasenha_similar_a_contrasenha_anterior(
    string $contrasenha,
    string $contrasenha_anterior,
): bool {
