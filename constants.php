...
define(
    "ASSETS",
    DIRNAME . DIRECTORY_SEPARATOR . "assets" . DIRECTORY_SEPARATOR,
);
define(
    "HELPERS",
    DIRNAME . DIRECTORY_SEPARATOR . "helpers" . DIRECTORY_SEPARATOR,
);
define(
    "COMPONENT_ADMIN",
    DIRNAME .
...
define("NAVES", [
    ...
    "naves" => [
        ...
        [
            "range" => ["NC120", "NC151", "MC001", "MC002"],
            "title" => "Nave2",
            "indice" => 2,
        ],
        [
            "range" => ["NC186", "NC217", "MC002", "MC003"],
            "title" => "Nave3",
            "indice" => 3,
        ],
        ...
        [
            "range" => ["NC152", "NC185", "MC005", "MC006"],
            "title" => "Nave6",
            "indice" => 6,
        ],
        [
            "range" => ["NC218", "NC251", "MC004", "MC005"],
            "title" => "Nave7",
            "indice" => 7,
        ],
...
$configuredAllowedHosts = get_env_value("APP_ALLOWED_HOSTS", $envVariables);
$allowedHosts = [];
if (is_string($configuredAllowedHosts) && $configuredAllowedHosts !== "") {
...
if (!defined("VIRUSTOTAL_API_KEY_FILE")) {
    define(
        ...
        DIRNAME . DIRECTORY_SEPARATOR . "virustotal_api_key.php",
    );
}

// Detectar el servidor
$servidor = $effectiveHost;
...
if ($isLocal) {
    $servidor_bd =
        get_env_value("PRACEAR_DB_HOST_LOCAL", $envVariables) ??
        get_env_value("PRACEAR_DB_HOST", $envVariables);
    $usuario =
        get_env_value("PRACEAR_DB_USER_LOCAL", $envVariables) ??
        get_env_value("PRACEAR_DB_USER", $envVariables);
    $clave =
        get_env_value("PRACEAR_DB_PASSWORD_LOCAL", $envVariables) ??
        get_env_value("PRACEAR_DB_PASSWORD", $envVariables);
    $bd =
        get_env_value("PRACEAR_DB_NAME_LOCAL", $envVariables) ??
        get_env_value("PRACEAR_DB_NAME", $envVariables);
} else {
    // En producción (Dinahosting), intentamos leer las variables estándar o las de PROD
    $servidor_bd =
        get_env_value("PRACEAR_DB_HOST", $envVariables) ??
        get_env_value("PRACEAR_DB_HOST_PROD", $envVariables);
    $usuario =
        get_env_value("PRACEAR_DB_USER", $envVariables) ??
        get_env_value("PRACEAR_DB_USER_PROD", $envVariables);
    $clave =
        get_env_value("PRACEAR_DB_PASSWORD", $envVariables) ??
        get_env_value("PRACEAR_DB_PASSWORD_PROD", $envVariables);
    $bd =
        get_env_value("PRACEAR_DB_NAME", $envVariables) ??
        get_env_value("PRACEAR_DB_NAME_PROD", $envVariables);
}
